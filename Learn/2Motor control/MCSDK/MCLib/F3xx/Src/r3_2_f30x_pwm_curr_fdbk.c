...
{
  R3_2_F30XOPAMPParams_t const * pDOPAMPParams_str = pHandle->pParams_str->pOPAMPParams;
  ...
  if ( ( uint32_t )pHandle == ( uint32_t )&pHandle->_Super )
  {
    ...
    LL_ADC_DisableIT_EOC( pHandle->pParams_str->ADCx_1 );
    LL_ADC_ClearFlag_EOC( pHandle->pParams_str->ADCx_1 );
    LL_ADC_DisableIT_JEOC( pHandle->pParams_str->ADCx_1 );
    LL_ADC_ClearFlag_JEOC( pHandle->pParams_str->ADCx_1 );
    LL_ADC_DisableIT_EOC( pHandle->pParams_str->ADCx_2 );
    LL_ADC_ClearFlag_EOC( pHandle->pParams_str->ADCx_2 );
    LL_ADC_DisableIT_JEOC( pHandle->pParams_str->ADCx_2 );
    LL_ADC_ClearFlag_JEOC( pHandle->pParams_str->ADCx_2 );
    ...
    else
    {
      ...
      pHandle->wADC_JSQR_phA = R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_1,
                               pHandle->pParams_str->bIaChannel, 1u, pHandle->pParams_str->b_IaSamplingTime, 1u, LL_ADC_INJ_TRIG_EXT_RISING,
                               ADC_ExternalTriggerInjected );
      pHandle->wADC_JSQR_phB = R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_2,
                               pHandle->pParams_str->bIbChannel, 1u, pHandle->pParams_str->b_IbSamplingTime, 1u, LL_ADC_INJ_TRIG_EXT_RISING,
                               ADC_ExternalTriggerInjected );
      pHandle->wADC_JSQR_phC = R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_2,
                               pHandle->pParams_str->bIcChannel, 1u, pHandle->pParams_str->b_IcSamplingTime, 1u, LL_ADC_INJ_TRIG_EXT_RISING,
                               ADC_ExternalTriggerInjected );
      R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_2, pHandle->pParams_str->bIaChannel, 1u,
                                           pHandle->pParams_str->b_IaSamplingTime, 1u, LL_ADC_INJ_TRIG_EXT_RISING, ADC_ExternalTriggerInjected );
      R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_1, pHandle->pParams_str->bIbChannel, 1u,
                                           pHandle->pParams_str->b_IbSamplingTime, 1u, LL_ADC_INJ_TRIG_EXT_RISING, ADC_ExternalTriggerInjected );
      R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_1, pHandle->pParams_str->bIcChannel, 1u,
                                           pHandle->pParams_str->b_IcSamplingTime, 1u, LL_ADC_INJ_TRIG_EXT_RISING, ADC_ExternalTriggerInjected );
    }

    LL_ADC_INJ_StopConversion( pHandle->pParams_str->ADCx_1 );
    LL_ADC_INJ_StopConversion( pHandle->pParams_str->ADCx_2 );
    pHandle->pParams_str->ADCx_1->JSQR = R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_1, 0u, 1u, 0u, 1u,
                                         LL_ADC_INJ_TRIG_EXT_RISING, ADC_ExternalTriggerInjected );
    pHandle->pParams_str->ADCx_2->JSQR = R3_2_F30X_ADC_InjectedChannelConfig( pHandle->pParams_str->ADCx_2, 0u, 1u, 0u, 1u,
                                         LL_ADC_INJ_TRIG_EXT_RISING, ADC_ExternalTriggerInjected );
...
void R3_2_F30X_CurrentReadingCalibration( PWMC_Handle_t * pHdl )
{
  ...
  if ( pDOPAMPParams_str )
  {
    ...
    pHandle->wOAMP1CR |= ( uint32_t )( pDOPAMPParams_str->wOPAMP_InvertingInput |
                                       pDOPAMPParams_str->wOPAMP_NonInvertingInput_PHB );
    pHandle->wOAMP2CR = OPAMP3->CSR;
    pHandle->wOAMP2CR &= ( uint32_t ) ( OPAMP_CSR_DEFAULT_MASK );
    pHandle->wOAMP2CR |= ( uint32_t )( pDOPAMPParams_str->wOPAMP2_InvertingInput |
                                       pDOPAMPParams_str->wOPAMP2_NonInvertingInput_PHA );
  }
  else
  {
  ...
  if ( pDOPAMPParams_str )
  {
    ...
    pHandle->wOAMP2CR &= ( uint32_t ) ( OPAMP_CSR_DEFAULT_MASK );
    pHandle->wOAMP2CR |= ( uint32_t )( pDOPAMPParams_str->wOPAMP2_InvertingInput |
                                       pDOPAMPParams_str->wOPAMP2_NonInvertingInput_PHC );
  }
  else
  {
...
uint16_t R3_2_F30X_SetADCSampPointSect1( PWMC_Handle_t * pHdl )
{
  ...
  if ( ( uint16_t )( pHandle->Half_PWMPeriod - pHandle->_Super.hCntPhA ) > pHandle->pParams_str->hTafter )
  {
    ...
    if ( pDOPAMPParams_str )
    {
      ...
      /* in case internal OPAMP, only the OPAMP input are switched, the ADC channels remain the same   */
      pHandle->wADC1_JSQR = pHandle->wADC_JSQR_phA + adcTrig;
      pHandle->wADC2_JSQR = pHandle->wADC_JSQR_phB + adcTrig;
    }
    else
    {
      pHandle->wADC1_JSQR = pHandle->wADC_JSQR_phB + adcTrig;
      pHandle->wADC2_JSQR = pHandle->wADC_JSQR_phA + adcTrig;
    }
  }
  else
  {
    ...
    {
      pHandle->wADC1_JSQR = pHandle->wADC_JSQR_phB + adcTrig;
      pHandle->wADC2_JSQR = pHandle->wADC_JSQR_phC + adcTrig;
    }
  }
  /* Set TIMx_CH4 value */
  TIMx->CCR4 = hCntSmp;
...
uint16_t R3_2_F30X_SetADCSampPointSect2( PWMC_Handle_t * pHdl )
{
  ...
  else
  {
    ...
    if ( pDOPAMPParams_str )
    {
      ...
      pHandle->wOAMP1CR &= ( uint32_t ) ( OPAMP_CSR_DEFAULT_MASK );
      pHandle->wOAMP1CR |= ( uint32_t )( pDOPAMPParams_str->wOPAMP_InvertingInput |
                                         pDOPAMPParams_str->wOPAMP_NonInvertingInput_PHA );
      pHandle->wOAMP2CR = OPAMP3->CSR;
      pHandle->wOAMP2CR &= ( uint32_t ) ( OPAMP_CSR_DEFAULT_MASK );
      pHandle->wOAMP2CR |= ( uint32_t )( pDOPAMPParams_str->wOPAMP2_InvertingInput |
                                         pDOPAMPParams_str->wOPAMP2_NonInvertingInput_PHC );
...
uint16_t R3_2_F30X_SetADCSampPointSect6( PWMC_Handle_t * pHdl )
{
  ...
  else
  {
    ...
    if ( pDOPAMPParams_str )
    {
      ...
      /* in case internal OPAMP, only the OPAMP inputs are switched, the ADC channels remain the same   */
      /* + adcTrig: trick to switch from default RISING edge (01) to FALLING edge (10) */
      pHandle->wADC1_JSQR = pHandle->wADC_JSQR_phA + adcTrig;
      pHandle->wADC2_JSQR = pHandle->wADC_JSQR_phB + adcTrig;
...
/**
  ...
  * @retval none
  */
void * R3_2_F30X_TIMx_UP_IRQHandler( PWMC_R3_2_F3_Handle_t * pHandle )
{
  R3_2_F30XOPAMPParams_t const *  pDOPAMPParams_str = pHandle->pParams_str->pOPAMPParams;
...
/**
  ...
  * @retval none
  */
void * R3_2_F30X_BRK2_IRQHandler( PWMC_R3_2_F3_Handle_t * pHandle )
{
...
/**
  ...
  * @retval none
  */
void * R3_2_F30X_BRK_IRQHandler( PWMC_R3_2_F3_Handle_t * pHandle )
{
