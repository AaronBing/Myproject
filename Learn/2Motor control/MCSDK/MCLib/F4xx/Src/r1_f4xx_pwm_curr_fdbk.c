...
static void R1F4XX_HFCurrentsCalibration( PWMC_Handle_t * pHdl, Curr_Components * pStator_Currents );
...
static void R1F4XX_TIMxInit( TIM_TypeDef * TIMx, TIM_TypeDef * TIMx_2, PWMC_Handle_t * pHdl )
{
  ...
  if ( ( pHandle->_Super.bMotor == M1 ) || ( pHandle->pParams_str->bFreqRatio == 2u ) )
  {
...
void R1F4XX_SwitchOffPWM( PWMC_Handle_t * pHdl )
{
  ...
  /* TODO : Give worth comments to these 4 lines of code ... .*/
  while ( LL_TIM_IsActiveFlag_UPDATE( pHandle->pParams_str->TIMx ) == 0 )
  {
    if ( LL_TIM_IsEnabledIT_UPDATE ( pHandle->pParams_str->TIMx ) )
    {
      break;
    }
  }
 ...
 * @retval none
 */
void * R1F4XX_TIM1_UP_IRQHandler( PWMC_R1_F4_Handle_t * pHandle )
{
 ...
 * @retval none
 */
void * R1F4XX_TIM8_UP_IRQHandler( PWMC_R1_F4_Handle_t * pHandle )
{
  uint8_t bInverted_pwm_new;
 ...
 * @retval none
 */
void * R1F4XX_BRK_IRQHandler( PWMC_R1_F4_Handle_t * pHandle )
{
  if ( ( pHandle->pParams_str->LowSideOutputs ) == ES_GPIO )
  {
    LL_GPIO_ResetOutputPin( pHandle->pParams_str->pwm_en_u_port, pHandle->pParams_str->pwm_en_u_pin );
 ...
 * @retval none
 */
void * R1F4XX_DMA_TC_IRQHandler( PWMC_R1_F4_Handle_t * pHandle )
{
  uint16_t hAux;
...
void R1F4XX_RLDetectionModeEnable( PWMC_Handle_t * pHdl )
{
  ...
  pHandle->_Super.pFctGetPhaseCurrents = &R1F4XX_RLGetPhaseCurrents;
  pHandle->_Super.pFctTurnOnLowSides = &R1F4XX_RLTurnOnLowSides;
  pHandle->_Super.pFctSwitchOnPwm = &R1F4XX_RLSwitchOnPWM;
  pHandle->_Super.pFctSwitchOffPwm = &R1F4XX_RLSwitchOffPWM;
...
void R1F4XX_RLDetectionModeDisable( PWMC_Handle_t * pHdl )
{
  ...
  if ( pHandle->_Super.RLDetectionMode == true )
  {
    ...
    pHandle->_Super.pFctGetPhaseCurrents = &R1F4XX_GetPhaseCurrents;
    pHandle->_Super.pFctTurnOnLowSides = &R1F4XX_TurnOnLowSides;
    pHandle->_Super.pFctSwitchOnPwm = &R1F4XX_SwitchOnPWM;
    pHandle->_Super.pFctSwitchOffPwm = &R1F4XX_SwitchOffPWM;
...
uint16_t R1F4XX_RLDetectionModeSetDuty( PWMC_Handle_t * pHdl, uint16_t hDuty )
{
  ...
  uint16_t hAux = pHandle->Half_PWMPeriod;
...
void R1F4XX_RLSwitchOnPWM( PWMC_Handle_t * pHdl )
{
  ...
  LL_ADC_INJ_SetSequencerLength  ( pHandle->pParams_str->ADCx_Inj,  LL_ADC_INJ_SEQ_SCAN_DISABLE );
  LL_TIM_SetTriggerOutput  ( pHandle->pParams_str->TIMx_2, LL_TIM_TRGO_UPDATE );
...
void R1F4XX_RLSwitchOffPWM( PWMC_Handle_t * pHdl )
{
  ...
  while ( LL_TIM_IsActiveFlag_UPDATE ( pHandle->pParams_str->TIMx ) == 0u )
  {
    if ( LL_TIM_IsEnabledIT_UPDATE( pHandle->pParams_str->TIMx ) )
    {
      break;
