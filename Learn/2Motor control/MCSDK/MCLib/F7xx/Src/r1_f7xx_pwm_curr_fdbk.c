...
static void R1F7XX_HFCurrentsCalibration( PWMC_Handle_t * pHdl, Curr_Components * pStator_Currents );
...
static void R1F7XX_RLGetPhaseCurrents( PWMC_Handle_t * pHdl, Curr_Components * pStator_Currents );
static void R1F7XX_RLTurnOnLowSides( PWMC_Handle_t * pHdl );
static void R1F7XX_RLSwitchOnPWM( PWMC_Handle_t * pHdl );
static void R1F7XX_RLSwitchOffPWM( PWMC_Handle_t * pHdl );
...
void R1F7XX_Init( PWMC_R1_F7_Handle_t * pHandle )
{
  ...
  if ( TIMx == TIM1 )
  {
    ...
    LL_DMA_SetMemoryAddress( DMA2, LL_DMA_STREAM_5, ( uint32_t ) & ( pHandle->wPreloadDisableActing ) );
    LL_DMA_SetPeriphAddress( DMA2, LL_DMA_STREAM_5, ( uint32_t )&TIM1->CCMR1 );
    LL_DMA_SetDataLength( DMA2, LL_DMA_STREAM_5, 1 );

    /* enable DMA1 Channel5 */
    LL_DMA_EnableStream( DMA2, LL_DMA_STREAM_5 );
    pHandle->PreloadDMAy_Chx = DMA2_Stream5;
  }
  ...
  else
  {
    ...
    LL_DMA_SetMemoryAddress( DMA2, LL_DMA_STREAM_1, ( uint32_t )&pHandle->wPreloadDisableActing );
    LL_DMA_SetPeriphAddress( DMA2, LL_DMA_STREAM_1, ( uint32_t )&TIMx->CCMR1 );
    LL_DMA_SetDataLength( DMA2, LL_DMA_STREAM_1, 1 );
    /* enable DMA2 Channel2 */
    LL_DMA_EnableStream( DMA2, LL_DMA_STREAM_1 );
    pHandle->PreloadDMAy_Chx = DMA2_Stream1;
  }
#endif
...
/**
  ...
  */
void R1F7XX_GetPhaseCurrents( PWMC_Handle_t * pHdl, Curr_Components * pStator_Currents )
{
  int32_t wAux;
  int16_t hCurrA = 0;
  int16_t hCurrB = 0;
...
void R1F7XX_SwitchOnPWM( PWMC_Handle_t * pHdl )
{
  ...
  pHandle->PreloadDMAy_Chx->CR &= ( uint16_t )( ~DMA_SxCR_EN );
  pHandle->DistortionDMAy_Chx->CR &= ( uint16_t )( ~DMA_SxCR_EN );
  pHandle->DistortionDMAy_Chx->NDTR = 2u;
...
/**
  ...
  * @retval none
  */
void * R1F7XX_TIMx_UP_IRQHandler( PWMC_R1_F7_Handle_t * pHandle )
{
...
/**
  ...
  * @retval none
  */
void * R1F7XX_BRK2_IRQHandler( PWMC_R1_F7_Handle_t * pHandle )
{
  return &( pHandle->_Super.bMotor );
}
...
/**
  ...
  * @retval none
  */
void * R1F7XX_BRK_IRQHandler( PWMC_R1_F7_Handle_t * pHandle )
{
  ...
  else
  {
    ...
    switch ( pHandle->bInverted_pwm_new )
    {
      case INVERT_A:
        ...
        pHandle->PreloadDMAy_Chx->PAR = ( uint32_t )( &( TIMx->CCMR1 ) );
        pHandle->hDmaBuff[1] = pHandle->_Super.hCntPhA;
      ...
      case INVERT_C:
        ...
        pHandle->PreloadDMAy_Chx->PAR = ( uint32_t )( &( TIMx->CCMR2 ) );
        pHandle->hDmaBuff[1] = pHandle->_Super.hCntPhC;
    ...
    /*active vector*/
    pHandle->PreloadDMAy_Chx->CR &= ( uint16_t )( ~DMA_SxCR_EN );
    pHandle->PreloadDMAy_Chx->NDTR = 1u;
    pHandle->PreloadDMAy_Chx->CR |= DMA_SxCR_EN;
...
void R1F7XX_RLDetectionModeEnable( PWMC_Handle_t * pHdl )
{
  ...
  pHandle->_Super.pFctGetPhaseCurrents = &R1F7XX_RLGetPhaseCurrents;
  pHandle->_Super.pFctTurnOnLowSides = &R1F7XX_RLTurnOnLowSides;
  pHandle->_Super.pFctSwitchOnPwm = &R1F7XX_RLSwitchOnPWM;
  pHandle->_Super.pFctSwitchOffPwm = &R1F7XX_RLSwitchOffPWM;
...
void R1F7XX_RLDetectionModeDisable( PWMC_Handle_t * pHdl )
{
  ...
  if ( pHandle->_Super.RLDetectionMode == true )
  {
    ...
    pHandle->_Super.pFctGetPhaseCurrents = &R1F7XX_GetPhaseCurrents;
    pHandle->_Super.pFctTurnOnLowSides = &R1F7XX_TurnOnLowSides;
    pHandle->_Super.pFctSwitchOnPwm = &R1F7XX_SwitchOnPWM;
    pHandle->_Super.pFctSwitchOffPwm = &R1F7XX_SwitchOffPWM;
